...
sudo curl -L -o /config/tftpboot/ipxe.efi http://boot.ipxe.org/ipxe.efi
sudo curl -L -o /config/tftpboot/undionly.kpxe http://boot.ipxe.org/undionly.kpxe
...
sudo cp /config/raspi-tftp/rpi4-uefi-ipxe/efi/boot/bootaa64.efi /config/tftpboot/ipxe-arm64.efi
